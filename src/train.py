...
def train(model, loader, device, epochs, lr, weight_decay, save_path, weights):
    ...
    criterion = nn.CrossEntropyLoss(weight=weights); best = float("inf")
    for epoch in range(epochs):
        model.train(); running = correct = seen = 0
        for x, y, _ in tqdm(loader, desc=f"epoch {epoch + 1}/{epochs}"):
            ...
            running += loss.item() * len(y); correct += (logits.argmax(1) == y).sum().item(); seen += len(y)
        avg = running / len(loader.dataset); acc = correct / max(seen, 1)
        print(f"loss={avg:.5f} accuracy={acc:.4f}")
        if avg < best: best = avg; torch.save(model.state_dict(), save_path)
    model.load_state_dict(torch.load(save_path, map_location=device, weights_only=True)); return model
...
def load_or_train(builder, path, loader, device, args, weights, label):
    model = builder(5).to(device)
    if path.exists() and not args.force_retrain:
        print(f"Loading saved {label}: {path}")
        model.load_state_dict(torch.load(path, map_location=device, weights_only=True)); return model
    print(f"Training {label}...")
    return train(model, loader, device, args.epochs, Config.lr, Config.weight_decay, path, weights)
...
def main():
    ...
    print(f"Classes: {classes}"); print(f"Split sizes: train={len(train_s)}, validation={len(val_s)}, test={len(test_s)}")
    ...
    light = load_or_train(build_lightweight, model_dir / "lightweight.pt", train_loader, device, args, weights, "lightweight model")
    expert = load_or_train(build_expert, model_dir / "expert.pt", train_loader, device, args, weights, "expert model")

    lp_v, y_v = predict(light, val_loader, device); ep_v, _ = predict(expert, val_loader, device)
    router_path = model_dir / "router.joblib"
    if router_path.exists() and not args.force_retrain:
        print(f"Loading saved router: {router_path}"); router = joblib.load(router_path)
    else:
